...
When invoked directly (e.g. `mmw:turing [codename]`), the codename is passed in your invocation context.

Before the Startup Mode Check and before reading any input files, run via Bash:
```
python mmw_tools.py sync_pull <codename>
```

After writing all output files (research.md, notes.md updates, fact-check-vN.md), run via Bash:
```
